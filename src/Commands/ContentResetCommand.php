...
class ContentResetCommand extends Command
{
    ...
    private const STUB_PATH = __DIR__ . '/../stubs/config/';

    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        // Reset config
        // app.php
        $stubContent = file_get_contents(self::STUB_PATH . 'app.stub');
        $targetFolderPath = base_path() . "/config/app.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // container.php
        $stubContent = file_get_contents(self::STUB_PATH . 'container.stub');
        $targetFolderPath = base_path() . "/config/container.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // database.php
        $stubContent = file_get_contents(self::STUB_PATH . 'database.stub');
        $targetFolderPath = base_path() . "/config/database.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // process.php
        $stubContent = file_get_contents(self::STUB_PATH . 'process.stub');
        $targetFolderPath = base_path() . "/config/process.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // redis.php
        $stubContent = file_get_contents(self::STUB_PATH . 'redis.stub');
        $targetFolderPath = base_path() . "/config/redis.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // server.php
        $stubContent = file_get_contents(self::STUB_PATH . 'server.stub');
        $targetFolderPath = base_path() . "/config/server.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // session.php
        $stubContent = file_get_contents(self::STUB_PATH . 'session.stub');
        $targetFolderPath = base_path() . "/config/session.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // translation.php
        $stubContent = file_get_contents(self::STUB_PATH . 'translation.stub');
        $targetFolderPath = base_path() . "/config/translation.php";
        $this->checkAndCreateFile($targetFolderPath, $stubContent, true);

        // view.php
        $stubContent = file_get_contents(self::STUB_PATH . 'view.stub');
